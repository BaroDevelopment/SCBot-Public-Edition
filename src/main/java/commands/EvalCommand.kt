...
// MOST OF THE CODE IS FROM https://github.com/Xirado/Bean
// IT HAS BEEN ONLY SLIGHTLY MODIFIED

suspend fun eval(event: MessageReceivedEvent) {
    if (event.author.id !in SCBot.WHITELISTED_USERS) return
...
class EvalCommand {
    ...
    private suspend fun waitForEdit(jda: JDA, messageId: Long, content: String, bindings: Map<String, Any>) {
        val buttonEvent = jda.await<ButtonInteractionEvent>() {
            if (it.user.id !in SCBot.WHITELISTED_USERS) {
        ...
        val modalEvent = jda.await<ModalInteractionEvent> { it.modalId == "eval-$messageId" } ?: return
